...
#include <string.h> // strlen, strcpy, strcat
  ...
  void DK_DrawColorPicker(ImUI* io, Vector2 pos, Color* result, bool* is_open)
  {
    ...
    if (CheckCollisionPointRec(mousePos, toggleButtonRect)) {
      const char* text = "Toggle Color Picker"; //XXTH
    ...
    //XXTH DrawRectangleRoundedLinesEx from old DrawRectangleRoundedLines
      DrawRectangleRoundedLinesEx(selectionRect, 0.1, 10, 4.0, borderColor);
      ...
      DrawRectangleGradientV(hueRect.x,
                             ...
                             Fade(BLUE, 0.0),
                             Fade(BLUE, 1.0));
      DrawRectangleRoundedLinesEx(hueRect, 0.1, 10, 4.0, borderColor);
      ...
      DrawRectangleGradientV(alphaRect.x,
                             ...
                             Fade(WHITE, 0.0),
                             Fade(LIGHTGRAY, 1.0));
      DrawRectangleRoundedLinesEx(alphaRect, 0.1, 10, 4.0, borderColor);
                     ...
                     bool* focused)
  {
    ...
    DrawRectangleRoundedLinesEx(sliderBounds, 0.5f, 1, 2, borderColor); // outline
                    ...
                    const char* text)
  {
    ...
    DrawRectangleRoundedLinesEx(buttonBounds, 0.5f, 1, 2, borderColor); // outline
                                ...
                                ,int& cursorPos )
  {
    ...
    DrawRectangleRoundedLinesEx(buttonBounds, io->style->roundness, 1, io->style->borderSize, borderColor);
    ...
    if (*focused) {
      ...
      Rectangle selectionReactangle = { position.x + 8, position.y + 2.5f,
      ...
      if ((key >= 32) && (key <= 125)) {
        if (IsKeyDown(KEY_LEFT_SHIFT) || IsKeyDown(KEY_RIGHT_SHIFT)) {
          if (key >= 97 && key <= 122)
            key -= 32;
          /* XXTH this mapping does not work for me!
            ...
            switch (key) {
              case 59:
                key = 59; //58; XXTH ; becomes : orig: 58
                break;
              case 61:
                key = 61; //43; //XXTH WTF ? "=" => "+" with orig 43
                break;
              case 44:
                key = 60;
                break;
