...
//     RLAPI void UnloadImagePalette(Color *colors);                                                            // Unload colors palette loaded with LoadImagePalette()
    ...
    DefineEngineFunction( getTextureSize, String, (S32 textureId), , "get the texture size int width int height") {
        Texture2D* tex = TextureMap.get(textureId);
        if (!tex) return "0 0";

        static const U32 bufSize = 32;
        char* returnBuffer = Con::getReturnBuffer(bufSize);
        dSprintf(returnBuffer, bufSize, "%d %d", tex->height, tex->width);
        return returnBuffer;
    }
    // NOTE ELFSCRIPT wrapper
    ...
    // NOTE ELFSCRIPT wrapper
    DefineEngineFunction( getTextureHeight, S32, (S32 textureId), , "get the texture height ") {
        Texture2D* tex = TextureMap.get(textureId);
        if (!tex) return 0;
        return  tex->height;
    }
