...
function shapes_starfield_batch::Render(%this) {
    ...
    for (%i = 0; %i < %this.starCount; %i++)
    {
        ...
        %x = $x;
        %y = $y;
        %z = $z - %moveVal; // Update depth when setting local var **optimize**
        ...
        if (%z < 0.0 || %screenPos_X < 0.0 || %screenPos_Y < 0.0 || %screenPos_X > %scrWidth || %screenPos_Y > %scrHeight)
        {
            %x = GetRandomValue(%minH, %maxH);
            %y = GetRandomValue(%minW, %maxW);
            %z = 1.0;
    ...
    // %.drawCircles(%this.starCount, $YELLOW);
    if (%drawMode == 0) %this.drawLines(%this.starCount, "200 200 255 255");
    else if (%drawMode == 1) %this.drawRects(%this.starCount, "155 255 155");
    else %this.drawCircles(%this.starCount, "200 80 80");
